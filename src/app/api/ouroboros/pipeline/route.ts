...
function writeCookie(response: NextResponse, state: OuroborosState): void {
...
// ─── POST: Run input through 3-stage pipeline ────────────────
export async function POST(request: NextRequest) {
  try {
    ...
    const { groqKey, input, targetLanguage } = body;
    ...
    const rateCheck = checkRateLimit('ouroboros-pipeline', sessionId);
    ...
    // Update target language if player changed it
    if (targetLanguage && ['French', 'Spanish', 'German'].includes(targetLanguage)) {
      state.targetLanguage = targetLanguage as TargetLanguage;
    }
    ...
    // Update state
    state.apiCallCount += 3; // 3 API calls per pipeline run
    ...
    // If flag found, record solve time
