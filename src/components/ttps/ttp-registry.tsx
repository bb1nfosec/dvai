...
import { useSessionStore, type MutationEntry } from '@/store/session-store';

function generateMarkdown(mutations: MutationEntry[]): string {
  if (mutations.length === 0) return '# DVAI TTP Registry\n\nNo TTPs recorded yet.\n';
...
`;
  ...
  const entries = mutations.map((m) => `### ${m.ttpName}

- **Operation**: ${m.opCode}
- **Category**: \`${m.ttpCategory}\`
- **Hardening Level**: ${m.hardeningLevel}
- **Date Discovered**: ${m.createdAt}
...
export function TTPRegistry() {
  const mutations = useSessionStore((s) => s.mutations);

  const handleExport = () => {
    if (mutations.length === 0) return;
    const md = generateMarkdown(mutations);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => useSessionStore.setState({ mutations: [] })}
            className="gap-1.5"
          >
